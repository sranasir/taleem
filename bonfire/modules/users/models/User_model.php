...
class User_model extends BF_Model
{
    ...
    protected $validation_rules = array(
        ...
        array(
            'field' => 'IsPresident',
            'label' => 'lang:bf_ispresident',
            'rules' => 'trim|exact_length[1]|is_numeric',
        ),
        array(
            'field' => 'IsVerified',
            'label' => 'lang:bf_isverified',
            'rules' => 'trim|exact_length[1]|is_numeric',
        ),        
        array(
            'field' => 'jamat',
        ...
        array(
            'field' => 'gender',
            'label' => 'lang:bf_gender',
            'rules' => 'required|trim|exact_length[1]',
        ),
        array(
            'field' => 'home_phone',
            'label' => 'lang:bf_homephone',
            'rules' => 'trim|max_length[15]',
        ),
        array(
            'field' => 'cell_phone',
            'label' => 'lang:bf_cellphone',
            'rules' => 'required|trim|max_length[15]',
        ),              
        array(
            'field' => 'birth_date',
            'label' => 'lang:bf_birthdate',
            'rules' => 'required|trim',
        ),
        array(
            'field' => 'street_address',
            'label' => 'lang:bf_streetaddress',
        ...
        array(
            'field' => 'zip_code',
            'label' => 'lang:bf_zipcode',
            'rules' => 'required|trim|max_length[5]|is_numeric',
        ),        
    );
    ...
    public function prep_data($post_data)
    {
        ...
        if(isset($post_data['IsPresident']))
            $data['IsPresident'] = $post_data['IsPresident'];
        else
            $data['IsPresident'] = 0;
        if(isset($post_data['IsVerified']))
            $data['IsVerified'] = $post_data['IsVerified'];
        else
            $data['IsVerified'] = 0;
        
        $data['jamat'] = $post_data['jamat'];
        $data['AIMS'] = $post_data['AIMS'];
        $data['gender'] = $post_data['gender'];
        if(!empty($post_data['home_phone']))
            $data['home_phone'] = $post_data['home_phone'];
        $data['cell_phone'] = $post_data['cell_phone'];
        $data['street_address'] = $post_data['street_address'];
        $data['city'] = $post_data['city'];
        $data['state'] = $post_data['state'];
        $data['zip_code'] = $post_data['zip_code'];
   
        $data['birth_date'] = $post_data['birth_date'];
    ...
    public function set_activation($user_id)
    {
        ...
        switch ($activation_method) {
            ...
            case 1:
                ...
                $message            .= ' It might take a few minutes to reach your mailbox.';
