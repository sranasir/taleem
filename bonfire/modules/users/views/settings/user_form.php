...
?>
    ...
    <fieldset>
        ...
        <div class="control-group">
            <label class="control-label required" for="IsPresident_label"><?php echo "Jamat President"?></label>
            <div class="controls" role="group">
                <label class="radio" for="IsPresident_yes">
                    <input type="radio" name="IsPresident" id="IsPresident_yes" value="1"
                    <?php echo set_radio('IsPresident', "1", isset($user) && $user->IsPresident == "1"); ?> />
                    <?php echo "Yes" ?>
                </label>
                <label class="radio" for="IsPresident_no">
                    <input type="radio" name="IsPresident" id="IsPresident_no" value="0" 
                    <?php echo set_radio('IsPresident', "0", isset($user) && $user->IsPresident == "0"); ?> />
                    <?php echo "No" ?>
                </label>
            </div>
        </div>
        <div class="control-group">
            <label class="control-label required" for="IsVerified_label"><?php echo "Verified Member"?></label>
            <div class="controls" role="group">
                <label class="checkbox" for="IsVerified_yes">
                    <input type="checkbox" name="IsVerified" id="IsVerified" value="1"
                    <?php echo set_checkbox('IsVerified', "1", isset($user) && $user->IsVerified == "1"); ?> />
                </label>
            </div>
        </div>
        <br>
